...
3) 选项必须形成真实取舍：至少一个偏自保、一个偏争取公平；尽量不要出现所有数值同涨或同跌。
4) self只填基础 health/reputation/wealth 变化。
...
# 数值要求
1) 尽量不要出现所有数值同涨或同跌
2) health 范围建议在-8 ~ 8 内
3) reputation 范围建议在 -5 ~ 5 内
4) wealth单位为万元；普通事件 wealth 建议在 -2 ~ +2 内。
...
3) 成功效果：个人数值大幅提升，health建议+12 ~ +24，reputation建议+8 ~ +20，wealth建议 +8 ~ +30。
4) 失败效果：个人数值小幅损耗但不致死，health建议-2 ~ -8，reputation建议-1 ~ -6，wealth建议 -1 ~ -5。
...
4) self只填基础 health/reputation/wealth。
...
# 数值要求
1) 尽量不要出现所有数值同涨或同跌
2) health 范围建议在 0 ~ +5 内
3) reputation 范围建议在 0 ~ +2 内
4) wealth单位为万元；普通事件 wealth 建议在 -2 ~ 0 内。
...
# 生成要求
1) 所有选项 health 必须为正数。
2) 每个选项必须有代价：wealth<0 或 reputation<0，至少一个为负。
...
# 数值要求
1) 尽量不要出现所有数值同涨或同跌
2) health 范围建议在 +4 ~ +12 内
3) reputation 范围建议在 -8 ~ +2 内
4) wealth单位为万元；普通事件 wealth 建议在 -5 ~ 0 内。
...
6) 单次法庭投票的effects只填写self。
7) 每个选项写 voterProfile，描述选择该立场的人通常如何想。

# 数值要求
1) 尽量不要出现所有数值同涨或同跌
2) health 范围建议在 -5 ~ +5 内
3) reputation 范围建议在 -5 ~ +5 内
4) wealth单位为万元；普通事件 wealth 建议在 -2 ~ +2 内。
...
2) maleDelta 和 femaleDelta 是最近三轮对男性/女性社会权益值的综合变化，建议在 -10 ~ +10 范围内。
